...
class TiSpell_CINO_wo_Res(nn.Module):
    def __init__(self, model_name, tokenizer):
        super(TiSpell_CINO_wo_Res, self).__init__()
...
class TiSpell_CINO(nn.Module):
    def __init__(self, model_name, tokenizer):
        super(TiSpell_CINO, self).__init__()
        self.model = AutoModelForCausalLM.from_pretrained(model_name, output_attentions=True)
        self.vocab_size = len(tokenizer)
        self.model.resize_token_embeddings(self.vocab_size)
        self.config = self.model.config
...
class TiSpell_CINO_FC1(nn.Module):
    def __init__(self, model_name, tokenizer):
        super(TiSpell_CINO_FC1, self).__init__()
        self.model = AutoModelForCausalLM.from_pretrained(model_name, output_attentions=True)
        self.vocab_size = len(tokenizer)
        self.model.resize_token_embeddings(self.vocab_size)
        self.config = self.model.config
