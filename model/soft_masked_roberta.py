...
from transformers import BertTokenizer, BertModel, BertConfig, RobertaModel, AutoConfig
...
class SoftMaskedRoBERTa(nn.Module):
    def __init__(self, model_name, tokenizer, device, hidden=256, layer_n=1):
        super(SoftMaskedRoBERTa, self).__init__()
        self.model = RobertaModel.from_pretrained(model_name, output_attentions=True)
        self.vocab_size = len(tokenizer)
        self.model.resize_token_embeddings(self.vocab_size)
        self.embedding = self.model.embeddings.to(device)
        self.config = self.model.config
        embedding_size = self.config.to_dict()['hidden_size']

        self.detector = BiGRU(embedding_size, hidden, layer_n)
        self.corrector = self.model.encoder
    ...
    def forward(self, input_ids, attention_mask):
        e = self.embedding(input_ids=input_ids)
        p = self.detector(e)
        ...
        encoder_extended_attention_mask= self._init_inputs(input_ids, attention_mask)
        h = self.corrector(e_,
